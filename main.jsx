...
import {
  ...
  AlertTriangle, ChevronRight, DollarSign, Phone, MapPin, Minus,
  TrendingUp, Clock, CheckCircle2, Settings, Download, Upload,
  LogOut, Printer, Star, Lock,
} from 'lucide-react';
...
const fromDbNegocio = (r) => ({ nombre: r.nombre, direccion: r.direccion, telefono: r.telefono, alias: r.alias, instagram: r.instagram, pin: r.pin });
...
function ReceiptContent({ pedido, cliente, negocio }) {
    ...
    <div style={{ border: '2px solid #000', padding: 18, fontFamily: 'Arial, Helvetica, sans-serif', color: '#000', background: '#fff', maxWidth: 680, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '2px solid #000', paddingBottom: 10, marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src={LOGO_SRC} alt="logo" style={{ width: 44, height: 44, borderRadius: 6 }} />
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid #000', paddingBottom: 8, marginBottom: 8, fontSize: 12 }}>
        <div style={{ flex: 1, borderRight: '1px solid #000', paddingRight: 8 }}>
          <div>Domicilio: {negocio.direccion}</div>
          <div>TE: {negocio.telefono}</div>
        </div>
        <div style={{ flex: 1, paddingLeft: 8 }}>
          <div>Alias: {negocio.alias}</div>
          <div>Instagram: {negocio.instagram}</div>
        </div>
      </div>
      <div style={{ borderBottom: '2px solid #000', paddingBottom: 8, marginBottom: 8, fontSize: 12 }}>
        <div style={{ fontWeight: 700, marginBottom: 2 }}>SR.(ES):</div>
        <div>{cliente ? cliente.nombre : 'Consumidor final'}</div>
        {cliente && cliente.direccion && <div>{cliente.direccion}</div>}
        {cliente && cliente.telefono && <div>Tel: {cliente.telefono}</div>}
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, marginBottom: 16 }}>
        <thead>
          <tr style={{ borderBottom: '1px solid #000' }}>
            <th style={{ textAlign: 'left', padding: '4px 2px' }}>Codigo</th>
            <th style={{ textAlign: 'left', padding: '4px 2px' }}>Cantidad</th>
            <th style={{ textAlign: 'left', padding: '4px 2px' }}>Descripcion</th>
            <th style={{ textAlign: 'right', padding: '4px 2px' }}>Precio unitario</th>
        ...
        <tbody>
          {pedido.items.map((it, i) => (
            <tr key={i}>
              <td style={{ padding: '3px 2px' }}>{it.codigo || '-'}</td>
              <td style={{ padding: '3px 2px' }}>{it.cantidad}</td>
              <td style={{ padding: '3px 2px' }}>{it.nombre}</td>
              <td style={{ padding: '3px 2px', textAlign: 'right' }}>{it.precioUnitario.toFixed(2)}</td>
              <td style={{ padding: '3px 2px', textAlign: 'right' }}>{(it.cantidad * it.precioUnitario).toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      {pedido.notas && <div style={{ marginBottom: 12, fontSize: 11 }}>Notas: {pedido.notas}</div>}
  ...
  return (
    <Modal title="Vista previa del remito" subtitle="Asi se ve (y se imprime)" onClose={closeModal} wide>
      <div className="rounded-lg mb-4" style={{ background: '#DEE3E9', padding: 16, overflowX: 'auto' }}>
        <ReceiptContent pedido={pedido} cliente={cliente} negocio={negocio} />
      </div>
...
function App() {
  ...
  const ajustarStock = async (id, delta) => {
    const nuevoStock = Math.max(0, Math.round(((productos.find(p => p.id === id) || { stock: 0 }).stock + delta) * 100) / 100);
  ...
  /* ---- pedidos ---- */
  const aplicarDeltaStock = async (deltas) => {
    const nuevosPorId = {};
    deltas.forEach(d => {
      if (!d.delta) return;
      const prod = productos.find(p => p.id === d.productoId);
      if (!prod) return;
      nuevosPorId[d.productoId] = Math.max(0, Math.round((prod.stock + d.delta) * 100) / 100);
    });
    ...
    beginSave();
    await Promise.all(ids.map(id => supabase.from('productos').update({ stock: nuevosPorId[id] }).eq('id', id)));
    endSave();
  };
  ...
  const crearPedido = async ({ clienteId, empleadoId, fecha, items, notas }) => {
    ...
    await aplicarDeltaStock(items.map(it => ({ productoId: it.productoId, delta: -it.cantidad })));
  };
  ...
  const editarPedido = async (pedidoId, { clienteId, empleadoId, fecha, items, notas }) => {
    ...
    await aplicarDeltaStock(deltas);
  };
  ...
  const cambiarEstadoPedido = async (id, nuevoEstado) => {
    ...
    if (nuevoEstado === 'cancelado' && estadoAnterior !== 'cancelado') {
      await aplicarDeltaStock(p.items.map(it => ({ productoId: it.productoId, delta: it.cantidad })));
    }
  };
  ...
  const shared = {
    ...
    saveProducto, deleteProducto, ajustarStock,
...
function StockView({ productos, openModal, ajustarStock }) {
  const [q, setQ] = useState('');
            ...
            return (
              <div key={p.id} className="list-row flex items-center justify-between px-4 py-3.5 gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="mono text-sm font-semibold w-14 text-center" style={{ color: bajo ? 'var(--rust)' : cerca ? 'var(--amber)' : 'var(--ink)' }}>{p.stock}</span>
      ...
      {basePedidos.length === 0 ? (
        ...
        <div className="card rounded-xl">
          {filtered.map(p => {
            ...
            const est = ESTADOS[p.estado];
            return (
              <button key={p.id} onClick={() => openModal('pedidoDetail', { pedidoId: p.id })} className="list-row w-full flex items-center justify-between px-4 py-3.5 text-left">
                <div className="min-w-0">
                  <div className="text-sm font-semibold truncate" style={{ color: 'var(--ink)' }}>{cliente ? cliente.nombre : 'Cliente eliminado'}</div>
                  <div className="text-xs mono" style={{ color: 'var(--ink-2)' }}>REM-{String(p.numero).padStart(4, '0')} · {fmtDate(p.fecha)}{empleado ? ` · ${empleado.nombre}` : ''}</div>
                </div>
                <div className="text-right shrink-0 pl-3">
                  <div className="mono text-sm font-semibold mb-1">{money(p.total)}</div>
                  <Pill bg={est.bg} fg={est.fg}>{est.label}</Pill>
                </div>
              </button>
            );
      ...
      <div>
        ...
        {items.length > 0 && (
          <div className="mb-4 space-y-2">
            {items.map(it => {
              const excede = (parseFloat(it.cantidad) || 0) > it.stockDisponible;
              return (
                <div key={it.productoId} className="rounded-lg p-3" style={{ background: 'var(--surface-alt)' }}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="mono text-sm font-semibold w-20 text-right">{money((parseFloat(it.cantidad) || 0) * it.precioUnitario)}</span>
                    <button type="button" onClick={() => removeItem(it.productoId)} style={{ color: 'var(--rust)' }}><X size={16} /></button>
                  </div>
                  {excede && <p className="text-xs mt-1" style={{ color: 'var(--rust)' }}>Stock insuficiente (disponible: {it.stockDisponible} {it.unidad})</p>}
                </div>
              );
            })}
          </div>
...
function AjustesModal({ negocio, onSave, onExport, onImportClick, onClose }) {
  const [form, setForm] = useState({
